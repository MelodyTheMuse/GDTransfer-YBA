...
func _on_play(ring_type:beat_ring_button_resource.ring_types):
	match ring_type:
		...
		beat_ring_button_resource.ring_types.TROMPET:
			_set_player_volumes()
			trompet_ring_players[primary_player].play(0)
...
func _fill_players(_bank):
	...
	klap_ring_players.get(alternative_player).stream = _bank.klap_alt
	trompet_ring_players.get(primary_player).stream =_bank.snare
	trompet_ring_players.get(alternative_player).stream = _bank.snare_alt
